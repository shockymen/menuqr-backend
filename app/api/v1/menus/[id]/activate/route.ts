// app/api/v1/menus/[id]/activate/route.ts
...
// Activate a menu (make visible to customers)
  ...
  try {
    ...
    if (bizError || !business || business.user_id !== user.id) {
      return NextResponse.json<ApiResponse>({
        error: {
          code: 'FORBIDDEN',
          message: 'You do not have permission to activate this menu'
        }
      }, { status: 403 })
    }
    ...
    if (menu.is_active) {
      return NextResponse.json<ApiResponse>({
        data: {
          menu: menu as Menu,
          message: 'Menu is already active'
        }
      }, { status: 200 })
    }
    ...
    if (updateError) {
      console.error('Menu activation error:', updateError)
      return NextResponse.json<ApiResponse>({
        error: {
          code: 'UPDATE_FAILED',
    ...
    console.error('Activate menu error:', errorMessage)
